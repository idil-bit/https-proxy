...
void llmproxy_request(char *model, char *system, char *query, char *response_body){
    ...
    snprintf(request,
             ...
             request_fmt,
             model,
             system, // idil: can send in with "Concisely summarize the following Wikipedia page" for first requests, later can change into the q about followup questions
             query,
             0.0,
             0, // idil: can send in with 0 first, then 1 if followup question feature is implemented
             "GenericSession");
